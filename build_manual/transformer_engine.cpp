...
static ModelError_t addTensor_w_enc(QnnModel& model){
  ModelError_t err = MODEL_NO_ERROR;
  uint32_t dimensions_w_enc[] = {16, 16};
  VALIDATE(model.addTensor("w_enc", // Tensor Name
                           (Qnn_Tensor_t) {
                               .version= QNN_TENSOR_VERSION_2,
                               {.v2= {
                                 .id=0,
...
static ModelError_t addTensor_w_dec(QnnModel& model){
  ModelError_t err = MODEL_NO_ERROR;
  uint32_t dimensions_w_dec[] = {16, 16};
  VALIDATE(model.addTensor("w_dec", // Tensor Name
                           (Qnn_Tensor_t) {
                               .version= QNN_TENSOR_VERSION_2,
                               {.v2= {
                                 .id=0,
...
static ModelError_t addNode_node_matmul(QnnModel& model){
  ...
  uint32_t dimensions_matmul[] = {4, 16};
...
static ModelError_t addNode_node_matmul_1(QnnModel& model){
  ...
  uint32_t dimensions_matmul_1[] = {4, 16};
...
static ModelError_t addNode_node_matmul_2(QnnModel& model){
  ...
  uint32_t dimensions_matmul_2[] = {4, 4};
...
static ModelError_t addNode_node_softmax(QnnModel& model){
  ...
  uint32_t dimensions_softmax[] = {4, 4};
...
static ModelError_t addNode_node_matmul_3(QnnModel& model){
  ...
  uint32_t dimensions_prediction[] = {4, 16};
...
static ModelError_t addNode_node_matmul_4(QnnModel& model){
  ...
  uint32_t dimensions_grad_w_dec[] = {16, 16};
    ...
    (Qnn_Tensor_t) {
          ...
          {.v2= {
            ...
            .quantizeParams= { QNN_DEFINITION_UNDEFINED,
                               ...
                               {.scaleOffsetEncoding= {.scale= 0.0000000000000000f, .offset= 0}}},
            .rank= 2,
            .dimensions=dimensions_grad_w_dec,
            .memType= QNN_TENSORMEMTYPE_RAW,
            {.clientBuf= { .data=nullptr,
                           .dataSize=0}},
            .isDynamicDimensions= nullptr,
...
static ModelError_t addNode_node_matmul_5(QnnModel& model){
  ...
  uint32_t dimensions_grad_w_enc[] = {16, 16};
    ...
    (Qnn_Tensor_t) {
          ...
          {.v2= {
            ...
            .quantizeParams= { QNN_DEFINITION_UNDEFINED,
                               ...
                               {.scaleOffsetEncoding= {.scale= 0.0000000000000000f, .offset= 0}}},
            .rank= 2,
            .dimensions=dimensions_grad_w_enc,
            .memType= QNN_TENSORMEMTYPE_RAW,
            {.clientBuf= { .data=nullptr,
                           .dataSize=0}},
            .isDynamicDimensions= nullptr,
...
ModelError_t QnnModel_composeGraphs(Qnn_BackendHandle_t backendHandle,
  ...
  /* model/graph for qnn_transformer_model*/
  QnnModel qnn_transformer_model;
  const QnnGraph_Config_t** graphConfigs = nullptr;
  VALIDATE(getQnnGraphConfigFromInfo("qnn_transformer_model", graphsConfigInfo, numGraphsConfigInfo, graphConfigs), err);
  VALIDATE(qnn_transformer_model.initialize(backendHandle, interface, contextHandle, "qnn_transformer_model", debug, DO_GRAPH_NODE_VALIDATIONS, graphConfigs), err);
  VALIDATE(addTensor_src(qnn_transformer_model), err);
  VALIDATE(addTensor_tgt(qnn_transformer_model), err);
  VALIDATE(addTensor_w_enc(qnn_transformer_model), err);
  VALIDATE(addTensor_w_dec(qnn_transformer_model), err);
  VALIDATE(addNode_node_t(qnn_transformer_model), err);
  VALIDATE(addNode_node_matmul(qnn_transformer_model), err);
  VALIDATE(addNode_node_t_1(qnn_transformer_model), err);
  VALIDATE(addNode_node_matmul_1(qnn_transformer_model), err);
  VALIDATE(addNode_node_t_2(qnn_transformer_model), err);
  VALIDATE(addNode_node_matmul_2(qnn_transformer_model), err);
  VALIDATE(addNode_node_softmax(qnn_transformer_model), err);
  VALIDATE(addNode_node_matmul_3(qnn_transformer_model), err);
  VALIDATE(addNode_node_sub(qnn_transformer_model), err);
  VALIDATE(addNode_node_t_3(qnn_transformer_model), err);
  VALIDATE(addNode_node_matmul_4(qnn_transformer_model), err);
  VALIDATE(addNode_node_matmul_5(qnn_transformer_model), err);

  // Add all models to array to get graphsInfo
  QnnModel* models [] = {&qnn_transformer_model};
