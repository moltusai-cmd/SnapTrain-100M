    ...
    if (!inputFileList.empty()) {
      ...
      while (inputFileIndexOffset < totalCount) {
        ...
        if (StatusCode::SUCCESS == returnStatus) {
          ...
          // --- LE HACK DU 100% NPU (PING-PONG BUFFER) ---
          int num_steps = 50000;
          printf("\n>>> ENGAGING SNAPDRAGON AT 100% SPEED (%d steps) <<<\n", num_steps);
          ...
          for (int step = 0; step < num_steps; step++) {
              ...
              if (executeStatus != QNN_GRAPH_NO_ERROR) break;

              // FEEDBACK LOOP (SWAP)
              // Au lieu de memcpy, on pourrait changer les adresses, 
              // mais pour le PoC on va tenter un memcpy ultra-sécurisé 
              // uniquement si le NPU a relâché le buffer.
              if (step % 5000 == 0) printf("  [NPU] Activity: %d it/s...\n", step);
          }
          printf(">>> NPU PERFORMANCE TEST COMPLETED <<<\n");
          // --- END OF HACK ---
          if (StatusCode::SUCCESS == returnStatus) {
